...
struct mat *quadriplica(struct mat *src)
{
	if(src==NULL)
		return NULL;
	
	struct mat *tmp = new_mat(src->rows*2, src->cols*2);
	...
	for(r=0; r<tmp->rows; r++)
	{
		for(c=0; c<tmp->cols; c++)
		{
			size_t rsorg = r;
			if(rsorg>=src->rows)
			rsorg=rsorg-src->rows;
			size_t csorg = c;
			if(csorg>=src->cols)
			csorg=csorg-src->cols;
			
			tmp->data[r*tmp->cols+c]=src->data[rsorg*src->cols+csorg];
		}
	}
...
int main()
{
	...
	*/
	...
	struct mat *quad = quadriplica(myMat);
	
	
	//struct mat *trans=transCalc(myMat);
	mat_print(quad,stdout);
