...
list readFile(list l, const char *filename)
{
...
void soglia(int val, list k)
{
	...
	while(k!=NULL)
	{
		if(val<=k->value)
		{
		   l1=insertHead(k->value,l1);
		}
		else
		{
		   l2=insertHead(k->value,l2);
		}
		
		k=k->next;
	}
	
	printf("\nLa list che contiene i valori >= di %d e'",val);
	printList(l1);
	printf("\nLa list che contiene i valori <= di %d e'",val);
	printList(l2);
}
