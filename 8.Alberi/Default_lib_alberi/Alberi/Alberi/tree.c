...
int ContaDominanti(tree t)
{
	...
	if (!isEmpty(t)) 
	{
		...
		dom=dom+ ContaDominanti(left(t));
		dom=dom+ ContaDominanti(right(t));
...
int SearchKey(tree t, int k)
{
	/*if (isEmpty(t))
		return 0;
	if (t->value <= k)
		return 1 + SearchKey(t->left, k) + SearchKey(t->right, k);
	else
		return 0 + SearchKey(t->left, k) + SearchKey(t->right, k);
		*/
	...
	if (t->value <= k)
	{
		l = 1 + SearchKey(t->left, k);
		r = 0 + SearchKey(t->right, k);
	}
